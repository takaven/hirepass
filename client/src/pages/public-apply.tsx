...
export function PublicCandidateForm({ pass, config }: { pass?: PublicPass; config: PublicConfig }) {
  ...
  if (confirmation) return <GlassCard className="p-8 text-center sm:p-10" role="status"><CheckCircle className="mx-auto mb-4 h-10 w-10 text-[#42494D]"/><h2 className="text-2xl font-semibold">{confirmation.title}</h2><p className="mt-2 text-muted-foreground">{confirmation.detail}</p>{confirmation.candidatePassUrl && <Button className="mt-5" asChild><a href={confirmation.candidatePassUrl}>View application status</a></Button>}<p className="mt-5 text-sm text-muted-foreground">The hiring team will manage any next step. No automatic response time is promised.</p></GlassCard>;
