...
`HIREPASS_PASS_ID_PREFIX` is optional and defaults to `HP`. External Candidate and Stakeholder Passes support an optional customer logo through `HIREPASS_COMPANY_LOGO_URL` and one optional six-digit hexadecimal accent through `HIREPASS_COMPANY_ACCENT_COLOR`; invalid or missing accents fall back to restrained HirePass Signal Lime. AI-assisted review is disabled unless `HIREPASS_AI_ENABLED=true` and `ANTHROPIC_API_KEY` is configured. `HIREPASS_AI_MODEL` defaults to `claude-sonnet-5`; batch size and timeout may be bounded with `HIREPASS_AI_MAX_BATCH` and `HIREPASS_AI_TIMEOUT_MS`.
