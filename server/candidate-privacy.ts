...
export async function eraseCandidatePii(candidateId: number) {
  ...
  try {
    const candidateResult = await client.query<{ id: number; cv_file_path: string | null }>(
      "select id, cv_file_path from candidates where id = $1",
      [candidateId],
    );
    ...
    await client.query(
      `update candidate_documents set file_path = null, file_name = null, file_size = null,
    ...
    await client.query("update pass_candidates set ai_brief = null, selection_notes = null, rejection_notes = null, updated_at = now() where candidate_id = $1", [candidateId]);
    ...
    await client.query("update offers set decline_reason = null, negotiation_notes = null, updated_at = now() where pass_candidate_id in (select id from pass_candidates where candidate_id = $1)", [candidateId]);
