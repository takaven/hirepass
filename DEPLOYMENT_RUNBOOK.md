...
- **Public intake configuration:** set `HIREPASS_COMPANY_NAME`, `HIREPASS_PRIVACY_NOTICE_URL` and `HIREPASS_PRIVACY_NOTICE_VERSION`. Optionally set `HIREPASS_COMPANY_LOCATION`, `HIREPASS_CAREERS_CONTACT_EMAIL`, `HIREPASS_COMPANY_LOGO_URL` and one exact six-digit `HIREPASS_COMPANY_ACCENT_COLOR` for public display. Publish `/talent-pool` for general submissions and `/apply/<numeric vacancy id>` for a direct vacancy application.
